...
This project is configured to publish with `CNAME` set to `impostergame.thethlaing.info`.

DNS setup:

1. Create a `CNAME` record for `imposter` pointing to `thethlaing.github.io`.
2. In GitHub repo settings -> Pages -> Custom domain, enter `impostergame.thethlaing.info`.
